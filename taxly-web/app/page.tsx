...
// ── FELIRATOZOTT SZÁM ─────────────────────────────────────────
function Szam({ ertek, cimke, tooltip, nagy, szin }: {
  ertek: string; cimke: string; tooltip: string; nagy?: boolean; szin?: string
}) {
  return (
    <div className={styles.szamBlokk}>
      <div className={styles.szamCimke}>
        {cimke}
        <Tooltip szoveg={tooltip}/>
      </div>
      <div className={`${styles.szamErtek} ${nagy ? styles.szamNagy : ''}`}
...
// ── SLIDER ────────────────────────────────────────────────────
function Slider({ label, value, onChange, min, max, step, hint, tooltip }: {
  label: string; value: number; onChange: (v: number) => void
  min: number; max: number; step: number; hint?: string; tooltip: string
}) {
    ...
    <div className={`${styles.kartya} ${legjobb ? styles.kartyaLegjobb : ''}`}
      ...
      <div className={styles.kartyaFejlec} onClick={onToggle}>
        ...
        <div className={styles.kartyaFejlecJobb}>
          <div className={styles.nettoSor}>
            <span className={styles.nettoLabel}>
              Évi nettó
              <Tooltip szoveg="Az összes adó és járulék levonása után ténylegesen kézhez kapott éves összeg."/>
            </span>
            <span className={styles.kartyaNetto} style={{ color: e.szin }}>{fmtM(e.evi_netto)}</span>
          </div>
          <div className={styles.haviSor}>
            <span className={styles.haviLabel}>
              Havi nettó
              <Tooltip szoveg="Az évi nettó összeg 12 hónapra osztva – ennyit jelent havonta kézhez."/>
            </span>
            <span className={styles.kartyaHavi}>{fmt(e.havi_netto)}/hó</span>
          </div>
        </div>
      </div>

      <div className={styles.progressWrap}>
        <div className={styles.progressBar} style={{ width: `${pct}%`, background: e.szin }}/>
      </div>

      <div className={styles.kartyaMeta}>
        <span className={styles.metaElem}>
          Adóteher: <b>{fmt(e.evi_adoteher)}</b>
          <Tooltip szoveg="Az összes befizetett adó és járulék éves összege (SZJA, TB, szochó, HIPA stb.)."/>
        </span>
        <span className={styles.metaElem}>
          Effektív adóráta: <b>{fmtP(e.effektiv)}</b>
          <Tooltip szoveg="Az adóteher és a bevétel aránya – megmutatja, a bevételed hány százalékát fizeted be adóként."/>
        </span>
      </div>
        ...
        <div className={styles.kartyaReszlet}>
          {e.tetelek.map((t, i) => (
            <div key={i} className={styles.tetelSor}>
              <span className={styles.tetelNev}>{t.n}</span>
              <span className={styles.tetelErtek}>{fmt(t.v)}</span>
            </div>
          ))}
          <div className={styles.tetelOsszeg} style={{ '--szin': e.szin } as React.CSSProperties}>
            <span>Évi nettó</span>
      ...
      <button className={styles.kartyaToggle} onClick={onToggle} style={{ color: e.szin }}>
    ...
    <main className={styles.main}>
      ...
      <nav className={styles.nav}>
        <div className={styles.logo}>
          <div className={styles.logoDot}/>taxly
        </div>
        <a href="https://taxly.gumroad.com/l/diojj" target="_blank" className={styles.navCta}>
          Early Bird – 9 900 Ft →
        </a>
      </nav>
      ...
      {jovobeli_aktiv.length > 0 && (
        <div className={styles.container}>
          <div className={styles.valtozasBanner}>
            <span className={styles.valtozasTitle}>📅 Közelgő változások</span>
          ...
          <div className={styles.kalkGrid}>
            ...
            <div className={styles.inputPanel}>
              ...
              <Slider label="Évi bevétel" value={bev} onChange={setBev}
                ...
                tooltip="Az összes bevételed egy évben, ÁFA nélkül. Ha ÁFA-s vagy, a nettó összeget add meg."/>
              ...
              <Slider label="Igazolt évi költség" value={ktg} onChange={setKtg}
                ...
                tooltip="A vállalkozásod igazolt, számlás kiadásai egy évben. Csak a SZJA tételes és a Kft. kalkulációnál számít – KATA és átalányadónál nincs hatása."/>

              <div className={styles.panelCim} style={{ marginTop: 20 }}>Tevékenység</div>
              {[
                { v: 'alap',      l: 'Szolgáltatás / szellemi munka', k: '45% elismert ktg',
                  tip: 'Az átalányadónál elismert költséghányad: a bevétel 45%-a automatikusan levonható adóalap-csökkentőként. 2026-tól emelkedett 40%-ról.' },
                { v: 'kisker',    l: 'Kiskereskedelem',               k: '80% elismert ktg',
                  tip: 'Kiskereskedelmi tevékenységnél az átalányadó alapja a bevétel csak 20%-a – a 80% automatikusan levonható.' },
                { v: 'specialis', l: 'Taxi / mezőgazdaság',           k: '90% elismert ktg',
                  tip: 'Taxisofőr, mezőgazdasági tevékenységnél a bevétel 90%-a levonható – csak 10% után kell adózni.' },
              ].map(opt => (
                <div key={opt.v}
                  className={`${styles.tevOpt} ${tev === opt.v ? styles.tevOptAktiv : ''}`}
                  onClick={() => setTev(opt.v)}>
                  <div className={`${styles.radio} ${tev === opt.v ? styles.radioAktiv : ''}`}/>
                  <div className={styles.tevSzoveg}>
                    <div className={styles.tevLabel}>{opt.l}</div>
                    <div className={styles.tevKtg}>
                      {opt.k}
                      <Tooltip szoveg={opt.tip}/>
                    </div>
                  </div>
                </div>
              ))}
              ...
              <div className={styles.hoHint}>
                Hány hónapja vállalkozol idén?
                <Tooltip szoveg="Ha az évnek csak egy részében voltál vállalkozó, add meg, hány hónapig. A KATA fix adó és a TB-járulék arányosan csökken."/>
              </div>
              ...
              <div className={styles.kftToggle} onClick={() => setKft(!kft)}>
                ...
                <div>
                  <div className={styles.toggleLabel}>
                    Kft. összehasonlítása
                    <Tooltip szoveg="A Kft. alapítás könyvelőt igényel és magasabb fix költségekkel jár, de bizonyos bevételi szint felett adóhatékonyabb lehet. Az összehasonlítás minimálbéres tulajdonos-ügyvezetőre számol."/>
                  </div>
            ...
            <div className={styles.eredmenyPanel}>
              ...
              {megtak > 0 && (
                <div className={styles.megtak}>
                  <div>
                    <div className={styles.megtakLabel}>
                      Max. évi megtakarítás
                      <Tooltip szoveg="A legjobb és a legrosszabb adózási forma közötti különbség – ennyit jelent évente a helyes forma megválasztása."/>
                    </div>
                  ...
                  <div className={styles.megtakJobb}>
                    ...
                    <div className={styles.megtakForma}>{legjobb.emoji} {legjobb.forma}</div>
                    <div className={styles.megtakHavi}>
                      {fmt(legjobb.havi_netto)}/hó
                      <Tooltip szoveg="A legjobb adózási formával havonta kézhez kapott nettó összeg."/>
                    </div>
                  </div>
                </div>
              )}
              ...
              {/* ÁFA figyelmeztető */}
              {bev > K.AFA_HATAR && (
                <div className={styles.afaBanner}>
                  ⚠️ {fmtM(bev)} bevételnél átlépted az ÁFA-mentesség határát (20 M Ft/év) – ÁFA-s leszel.
                  <Tooltip szoveg="Ha a bevételed meghaladja a 20 M Ft-os éves határt, kötelezővé válik az ÁFA-s számlázás. 2027-től 22 M Ft, 2028-tól 24 M Ft lesz a határ."/>
                </div>
              )}
      ...
      <section className={styles.ctaSection}>
        <div className={styles.ctaInner}>
          ...
          {status === 'ok' ? (
            <div className={styles.siker}>🎉 Feliratkoztál! Megnyílik a Gumroad oldal...</div>
          ) : (
            <form onSubmit={feliratkozas} className={styles.form}>
              <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                placeholder="email@cimed.hu" className={styles.emailInput} required/>
              <button type="submit" className={styles.ctaGomb} disabled={status === 'loading'}>
